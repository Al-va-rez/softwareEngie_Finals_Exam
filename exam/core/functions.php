<?php
    ...
    function search_Logs($pdo, $searchQuery) {
        $sql = "SELECT * FROM activity_Logs WHERE CONCAT(activity, affected_Entity, done_By, user_Role, date_Committed) COLLATE utf8mb4_bin LIKE ?";
    ...
    function getAll_Messages_Received($pdo, $user) {
    ...
    function search_Messages_Received($pdo, $searchQuery, $user) {
        $sql = "SELECT * FROM messages WHERE CONCAT(sender, subject, message, date_Sent) COLLATE utf8mb4_bin LIKE ? AND recipient = ?";
    ...
    function getAll_Messages_Sent($pdo, $user) {
    ...
    function search_Messages_Sent($pdo, $searchQuery, $user) {
        $sql = "SELECT * FROM messages WHERE CONCAT(sender, subject, message, date_Sent) COLLATE utf8mb4_bin LIKE ? AND sender = ?";
    ...
    // create (send message)
    function send_Message($pdo, $sender, $recipient, $subject, $message) {
        $response = array();

        $sql = "INSERT INTO messages (sender, recipient, subject, message) VALUES (?,?,?,?)";   

        $query = $pdo->prepare($sql);
        $message_Sent = $query->execute([$sender, $recipient, $subject, $message]);
    ...
    // specific message
    function getSepecific_Message($pdo, $msg_id) {
        $sql = "SELECT * FROM messages WHERE id = ?";

        $query = $pdo->prepare($sql);
        $executeQuery = $query->execute([$msg_id]);
    ...
    // delete
    function delete_Message($pdo, $msg_id) {

        $response = array();

        $log_Sql = "SELECT * FROM message WHERE id = ?";
        
        $log_Query = $pdo->prepare($log_Sql);
        $log_Query->execute([$msg_id]);

        $job_To_Delete = $log_Query->fetch();


        $activity_Log = log_Activity($pdo, "Deleted a message", $job_To_Delete['title'], $_SESSION['username'], $_SESSION['role']);
        ...
        if ($activity_Log) {
            ...
            $response = array(
                "status"=>"400",
                "message"=>"Something went wrong. Message could not be deleted."
            );
        }
    ...
    /* --- REPLIES --- */
    // get all replies received
    function getAll_Replies($pdo, $recipient, $original_Message) {
        $sql = "SELECT * FROM replies WHERE recipient = ? AND original_Message = ?";

        $query = $pdo->prepare($sql);
        $executeQuery = $query->execute([$recipient, $original_Message]);
    ...
    function search_Replies($pdo, $searchQuery, $user, $original_Message) {
        $sql = "SELECT * FROM replies WHERE CONCAT(sender, original_Message, reply date_Sent) COLLATE utf8mb4_bin LIKE ? AND recipient = ?";

        $query = $pdo->prepare($sql);
        $executeQuery = $query->execute(["%" . $searchQuery . "%", $user, $original_Message]);
    ...
    // create (send reply)
    function send_Reply($pdo, $sender, $recipient, $original_Message, $reply) {
        $response = array();

        $sql = "INSERT INTO replies (sender, recipient, original_Message, reply) VALUES (?,?,?,?)";   

        $query = $pdo->prepare($sql);
        $message_Sent = $query->execute([$sender, $recipient, $original_Message, $reply]);

        if ($message_Sent) {

            $activity_Log = log_Activity($pdo, "Sent a reply", $recipient, $sender, $_SESSION['role']);
            ...
            $response = array(
                "status"=>"400",
                "message"=>"Something went wrong. Reply could not be sent."
            );
        }
    ...
    // specific reply
    function getSepecific_Reply($pdo, $reply_id) {
        $sql = "SELECT * FROM replies WHERE id = ?";

        $query = $pdo->prepare($sql);
        $executeQuery = $query->execute([$reply_id]);
    ...
    // delete
    function delete_Reply($pdo, $reply_id) {

        $response = array();

        $log_Sql = "SELECT * FROM replies WHERE id = ?";
        
        $log_Query = $pdo->prepare($log_Sql);
        $log_Query->execute([$reply_id]);

        $log_values = $log_Query->fetch();


        $activity_Log = log_Activity($pdo, "Deleted a reply", $log_values['recipient'], $_SESSION['username'], $_SESSION['role']);
        ...
        if ($activity_Log) {
            ...
            $response = array(
                "status"=>"400",
                "message"=>"Something went wrong. Reply could not be deleted."
            );
        }
