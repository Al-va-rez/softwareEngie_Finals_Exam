<?php
    ...
    function getAll_Logs($pdo) {
        $sql = "SELECT * FROM activity_logs ORDER BY date_Committed DESC";
    ...
    function update_Application($pdo, $status, $remarks, $applicant, $job_Title) {
        $response = array();
        ...
        $sql = "UPDATE applications
                SET
                    status = ?,
                    remarks = ?
                WHERE applicant = ? AND job_Title = ?";   

        $query = $pdo->prepare($sql);
        $application_Updated = $query->execute([$status, $remarks, $applicant, $job_Title]);

        if ($application_Updated) {

            if ($status == 'Accepted') {
                $activity_Log = log_Activity($pdo, "Accepted Applicant", $applicant, $_SESSION['username'], $_SESSION['role']);
            } else {
                $activity_Log = log_Activity($pdo, "Rejected Applicant", $applicant, $_SESSION['username'], $_SESSION['role']);
            }
    ...
    /* --- MESSAGES --- */
    // get all messages
    function getAll_Subject_Messages($pdo, $subject) {
        $sql = "SELECT * FROM messages WHERE subject = ?";

        $query = $pdo->prepare($sql);
        $executeQuery = $query->execute([$subject]);
    ...
    function delete_Message($pdo, $msg_id) {
        ...
        $log_Sql = "SELECT * FROM messages WHERE id = ?";
        ...
        $activity_Log = log_Activity($pdo, "Deleted a message", $job_To_Delete['recipient'], $_SESSION['username'], $_SESSION['role']);
